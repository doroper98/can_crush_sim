...
export function getCategory(key: string): { label: string; color: string } {
  ...
  if (key === 'polycarbonate' || key.startsWith('peek') || key.startsWith('nylon_') || key === 'abs' || key === 'uhmwpe' || key === 'ptfe' || key === 'pom' || key.startsWith('pp') || key === 'hdpe' || key === 'ldpe' || key === 'pet' || key === 'polyimide' || key === 'epoxy' || key === 'pvdf' || key.startsWith('cfrp') || key.startsWith('gfrp') || key === 'psu' || key === 'pei' || key === 'etfe' || key === 'pai') return { label: 'Poly', color: '#facc15' }
