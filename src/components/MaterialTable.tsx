...
export function getCategory(key: string): { label: string; color: string } {
  ...
  if (key === 'polycarbonate' || key === 'peek' || key.startsWith('nylon_') || key === 'abs' || key === 'uhmwpe' || key === 'ptfe' || key === 'pom' || key === 'pp' || key === 'hdpe' || key === 'pet' || key === 'polyimide' || key === 'epoxy') return { label: 'Poly', color: '#facc15' }
